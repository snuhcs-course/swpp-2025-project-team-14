...
def mock_question_repo(mocker):
    repo = mocker.Mock()
    repo.create_question = mocker.MagicMock(
        side_effect=lambda user_id, question_type, text: text
    )
    repo.get_question_by_id.return_value = mocker.Mock(
        text="치킨을 먹으며 느낀 행복은 무엇 때문이었나요?"
    )
    return repo
...
def mock_journal_repo(mocker):
    repo = mocker.Mock()
    repo.list_journals_by_user.return_value = [
        mocker.Mock(content="오늘은 치킨을 먹었다.")
    ]
    return repo
...
def mock_answer_repo(mocker):
    repo = mocker.Mock()
    repo.get_answer_by_id.return_value = mocker.Mock(
        text="내 돈으로 벌어 내가 산 치킨을 먹으며 나 자신이 증명됨을 느꼈기 때문입니다."
    )
    repo.get_answer_by_question.return_value = mocker.Mock(
        text="내 돈으로 벌어 내가 산 치킨을 먹으며 나 자신이 증명됨을 느꼈기 때문입니다."
    )
    return repo
...
def mock_value_score_repo(mocker):
    repo = mocker.Mock()
    repo.get_top_5_value_scores.return_value = [
        mocker.Mock(polarity=1, value="친절", intensity=90, category="Neuroticism"),
        mocker.Mock(polarity=1, value="사교성", intensity=90, category="Extraversion"),
        mocker.Mock(
            polarity=-1, value="이기성", intensity=85, category="Openness to Experience"
        ),
...
def mock_value_map_repo(mocker):
    ...
    repo.get_by_user.return_value = mocker.Mock(
        score_0=11.1, score_1=22.2, score_2=33.3, score_3=44.4, score_4=55.5
    )
    repo.generate_comment = mocker.MagicMock(
        side_effect=lambda user_id, personality_insight, comment: (
            personality_insight,
            comment,
        )
    )
...
def question_service(mock_journal_repo, mock_question_repo):
    return QuestionService(
        journal_repository=mock_journal_repo, question_repository=mock_question_repo
    )


@pytest.fixture
def value_score_service(
    mock_question_repo, mock_answer_repo, mock_value_score_repo, mock_value_map_repo
):
...
def test_extract_value_score_from_answer(value_score_service):
    ...
    for result_element in result:
        assert result_element.category_key in [
            "Neuroticism",
            "Extraversion",
        ...
        assert isinstance(result_element.evidence, str)
