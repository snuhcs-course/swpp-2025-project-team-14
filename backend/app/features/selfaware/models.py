...
class Question(Base):
    ...
    )  # single_category | multi_category | personalized_category
    text: Mapped[str] = mapped_column(Text, nullable=False)

    date: Mapped[date] = mapped_column(
        Date, default=lambda: get_korea_time().date(), nullable=False
    )
...
class Answer(Base):
    ...
    keywords: Mapped[list[str] | None] = mapped_column(JSON, nullable=True)  # 확인 필요
...
class ValueScore(Base):
    ...
    )  # Doc에 명시가 안되어 있는데, service에서 사용할 가능성 있어 남겨둠, 확정시 문서에 추가
    ...
    polarity: Mapped[int] = mapped_column(
        Integer, nullable=False, default=1
    )  # -1, 0, +1       # 서버 계산치(즉시/증분)

    # 증거(문장/구) — 최대 2개 저장 추천
    evidence_quotes: Mapped[list[str] | None] = mapped_column(JSON)
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), default=get_korea_time, nullable=False
    )
