# SWPP Team 14 - Iteration 3 Working Demo

This repository contains the working demo of iteration 3.  
The demo demonstrates the backend API implementation of user authorization, journal features, self aware features tested in a local Docker-based environment. AWS EC2 connection and deployment successfully completed. The backend API is now accessible through the EC2 public endpoint(only user authorization and journal features). In android client, user can try sign up new account and log in feature, then use main features(journals, self aware, etc).
...
- **Android UI Updates:** Enhanced UI to support new self-aware, analysis, statistics features.
...
- Implemented APIs about **Self-Aware Q&A** features
- Enhanced Android client UI to support new features  

Next steps include:

- **Statistics & Analysis API implementation**
  - Finalize endpoints for emotion trends, keyword summaries, ValueMap aggregation, and personality insights.
  - Define response schemas (pagination, date range filters) and error codes.
  - Add background jobs for scheduled re-aggregation.
- **Psychological theory–grounded analysis** for self aware (e.g., Five‑Factor Model/Big Five)
- Wire Android **Statistics**/**Analysis** tabs to the APIs.s
- **Testing (Unit & Integration)**
  - **Unit tests (backend):** pytest for routers/services/repositories with in‑memory or tmp DB fixtures; mock external services (OpenAI, S3).
  - **Integration tests (backend):** FastAPI end‑to‑end tests for key flows (auth → journal → self‑aware → analyze & statistic).
  - **Android tests:** `JUnit`,`HiltAndroidTest` for ViewModels/Repositories, `Espresso`,`MockWebServer` for API contracts, and basic UI flows.
